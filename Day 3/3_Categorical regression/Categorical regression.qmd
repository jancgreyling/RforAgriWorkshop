...
```{r}
# Filter data for 'Died' == "No" and remove rows with missing values in 'Birthweight' or 'Apgar_score'
filtered_data <- subset(chap10data4, Died == "No" & !is.na(Birthweight) & !is.na(Apgar_score))

# Plot the data
#plot(x = filtered_data$Apgar_score, 
...
```{r}
chap10data4$Died <- chap10data4$Died <- factor(chap10data4$Died, 
                                               levels = c("No", "Yes"))
chap10data4.model1 <- glm(as.factor(Died) ~ ., family=binomial(), data=chap10data4)
...
```{r}
predict(chap10data4.model2,newdata=data.frame(Delivery.mode="Caesarean",Birthweight=1000,Apgar_score=6),type="response")
```
