...
manipulate(
  ...
  plot(x,exp(beta0+beta1*x) / (1+exp(beta0+beta1*x)),
       type="l",lwd=3,frame=FALSE,ylim=c(0,1)),
...
manipulate(
  plot(x=x,y=beta0+beta1*x, xlim = c(-15, 15), type = "l", ylim=c(-20,20)),
  beta1 = slider(-5,5,step=0.5,initial=2),
  beta0 = slider(-10,10,step=1,initial=-1)
...
```{r}
chap10data1 <- read.csv("chap10data1.csv", sep=",", header=T)
chap10data2 <- read.csv("chap10data2.csv", sep=",", header=T)
chap10data3 <- read.csv("chap10data3.csv", sep=",", header=T)
chap10data4 <- read.csv("chap10data4.csv", sep=",", header=T)
View(chap10data1)
View(chap10data2)
View(chap10data3)
View(chap10data4)
...
```{r}
unique(chap10data3$Died)
chap10data3$Died <- ifelse(chap10data3$Died == "Yes", 1, 0)
...
```{r}
influence.measures(chap10data3.model1)
influence.cutoffs(chap10data3.model1)
```
...
chap10data3$preds <- ifelse(chap10data3$probs>=0.2,"Yes","No")
table(chap10data3[,c("Died","preds")])
...
```{r}
# Filter data for 'Died' == "No" and remove rows with missing values in 'Birthweight' or 'Apgar.score'
filtered_data <- subset(chap10data4, Died == "No" & !is.na(Birthweight) & !is.na(Apgar.score))

# Plot the data
#plot(x = filtered_data$Apgar.score, 
    # y = filtered_data$Birthweight, 
     #col = "blue", 
     #xlab = "Apgar Score", 
     #ylab = "Birthweight", 
     #main = "Birthweight vs Apgar Score (Died = No)")
```


```{r}
filtered_data <- subset(chap10data4, Died == "No" & !is.na(Birthweight) & !is.na(Apgar_score))
plot(y=filtered_data$Birthweight,
     x = filtered_data$Apgar_score,col="blue",ylab="Weight",xlab="Apgar")
points(y=chap10data4[chap10data4$Died=="Yes",]$Birthweight,
     x=chap10data4[chap10data4$Died=="Yes",]$Apgar_score,col="red")

scatterplotMatrix(~Birthweight + Apgar_score + Temperature,data=chap10data4,
                  groups=chap10data4$Died,
                  main="ScatterPlot Matrix of Chapter 4 Dataset 4")
```

#fit logistic regression model

```{r}
chap10data4$Died <- ifelse(chap10data3$Died == "Yes", 1, 0)
...
```{r}
chap10data4$Died <- factor(chap10data4$Died, levels = c("No", "Yes"))
```


```{r}
# Fit the logistic regression model
chap10data4.model2 <- glm(Died ~ Birthweight + Apgar_score, family = binomial(), data = chap10data4)

# Calculate predicted probabilities
chap10data4$Probs <- predict(chap10data4.model2, type = "response")

# Generate predictions based on a threshold of 0.2
chap10data4$Preds <- ifelse(chap10data4$Probs >= 0.2, "Yes", "No")

# Convert predictions to a factor
chap10data4$Preds <- factor(chap10data4$Preds, levels = c("No", "Yes"))
```
...
```{r}
predict(chap10data4.model2,newdata=data.frame(Delivery.mode="Caesarean",Birthweight=1000,Apgar.score=6),type="response")
```
